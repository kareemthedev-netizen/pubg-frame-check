...
print("🖼️ AI Image Fixer (Enhanced) Started -", datetime.now())
...
def search_google_image(brand, model):
    API_KEY = os.environ.get('TAVILY_API_KEY')
    if not API_KEY:
        print("   ⚠️ No Tavily API key")
        return None
...
def search_duckduckgo(brand, model):
    ...
    try:
        ...
        # البحث عن الصور في الـ RelatedTopics
...
def fix_all_images(db):
    ...
    for doc in docs:
        ...
        if not check_image(image_url):
            ...
            if not new_image:
                new_image = search_google_image(device.get('brand'), device.get('model'))
                if new_image:
                    tavily_used += 1
